...
function getFile(file) {
  ...
  return function thunk(cb) {
    if (resp) {
      // 这里如果resp存在，说明ajax已经完成resp是参数
      cb(resp);
    } else {
      // resp不存在，就把回调函数赋给resp
      resp = cb;
    }
  };
}
    ...
    fetch(`/api/request/${project}`)
      ...
      .finally(() => {
        ...
        dispatch({
          ...
          payload:
            reqStartThisTime > reqStartLastTime
              ? reqStartThisTime
              : reqStartLastTime,
        });
      });
...
*/
export const fetchData1 = (project) => {
  return (dispatch, getState) => {
    dispatch({
      type: FETCH_BEGIN,
    });
...
search.pipe(switchMap(fectchData)).subscribe(console.log);
