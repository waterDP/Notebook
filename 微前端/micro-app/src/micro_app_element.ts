...
import { ObservedAttrName, lifeCycles, appStates } from "./constants";
...
export function defineElement(tagName: string): void {
  class MicroAppElement
    extends HTMLElement
    implements MicroAppElementInterface
  {
    ...
    public attributeChangedCallback(
      ...
      if (
        ...
        if (
          ...
          (!this.appUrl || !this.connectStateMap.get(this.connectedCount))
        ) {
          ...
          (!this.appName || !this.connectStateMap.get(this.connectedCount))
        ) {
      ...
      if (appInstanceMap.has(this.appName)) {
        ...
        if (oldApp.isHidden() && oldApp.url === this.appUrl) {
          ...
          if (__DEV__ && this.sameCoreOptions(oldApp)) {
            ...
            logWarn(
              `the ${
                oldApp.isPrefetch ? "prefetch" : "unmounted"
              } app with url ${oldAppUrl} replaced by a new app with url ${targetUrl}`,
              this.appName
            );
          }
          ...
          logError(
            `app name conflict, an app named ${this.appName} with url ${oldAppUrl} is running`
          );
        }
      } else {
    ...
    private getDisposeResult<T extends keyof OptionsType>(name: T): boolean {
      return (
        (this.compatibleProperties(name) || !!microApp.options[name]) &&
        this.compatibleDisableProperties(name)
      );
    }
    ...
    private useScopecss(): boolean {
      return !(
        this.getDisposeResult("disable-scopecss") ||
        this.getDisposeResult("shadowDOM")
      );
    }
    ...
    private updateSsrUrl(baseUrl: string): void {
      if (this.getDisposeResult("ssr")) {
        ...
        if (
          this.getDisposeResult("disable-memory-router") ||
          this.getDisposeResult("disableSandbox")
        ) {
          const rawLocation = globalEnv.rawWindow.location;
          this.ssrUrl = CompletionPath(
            rawLocation.pathname + rawLocation.search,
            baseUrl
          );
        } else {
    ...
    private getMemoryRouterMode(): string {
      return initRouterMode(
        ...
        this.compatibleProperties("disable-memory-router") &&
          this.compatibleDisableProperties("disable-memory-router")
      );
    }
    ...
    public getRouterEventDelay(): number {
      ...
      if (isNaN(delay)) {
        delay = parseInt(
          (isFunction(microApp.options["router-event-delay"])
            ? microApp.options["router-event-delay"](this.appName)
            : microApp.options["router-event-delay"]) as unknown as string
        );
      }
