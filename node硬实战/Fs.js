... 文件锁
	/**
	 * 文件锁在你需要协同多个进程同时访问一个文件并且要保证文件的完整性以及数据不能丢失的时候很有用
	 */
	> 使用独占标记创建锁文件
	/**
	 * fs 模块为所有需要打开的文件的方法（比如fs.writeFile/ fs.createWriteStream , 以及fs.open）
	 * 提供了一个x标记。这告诉操作系统这个文件应该以独占模式打开(O_EXCL)。
	 * 当使用这个方法时，若这个文件存在，文件不能被打开。
	 */
	fs.open('config.lock' , 'wx', err => {   // 以可执行、可写入的模式打开
	 ...
	 * 但最好把当前进程的进程号写到这个锁文件中而不是写一个空文件，
	 * 这样，当有异常发生时，我们知道最后拥有这个锁的进程
	 */
